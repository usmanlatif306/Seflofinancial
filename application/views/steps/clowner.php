...
<!--[if gt IE 8]><!-->
<head>
<title>Owner Information | Briskloanz </title>
 <?php include('top.php'); ?>
<style>
	.own-form{
	width: 600px;
		...
		<form method="post" class="own-form" name="step2form" id="step2form">
			<div class="row">
				...
				<!--<div class="form-group col-md-12">
						...
						<input type="password" class="form-control" maxlength="6" name="<?php //echo $question_id;?>" id="password" value="<?php echo $answers[$question_id]['answer']?$answers[$question_id]['answer']:''; ?>" >
				</div>
				<div class="form-group col-md-12">
						<?php //$question_id = $questions[5]['question_id']; ?>
						<label for="pwd"><?php //echo $questions[5]['question']; ?></label>
						<input type="password" class="form-control" maxlength="6" name="<?php //echo $question_id;?>" id="confirm_password" value="<?php echo $answers[$question_id]['answer']?$answers[$question_id]['answer']:''; ?>">
				</div>-->
		  ...
		  //  $('form#step2form').on('submit', function(e) {
		    	 ...
		    	 if($('#password').val().length>=6){
		    	   //to check pwd/confirm pwd same or not
			        ...
			        $.ajax({
			            type: 'post',
			            url: '<?php echo base_url();?>steps/addClOwner',
			            data: form,
			                ...
			                if (obj.status == 'success') {
			                  ...
			                  // $('.success_message').html(obj.message); 
			                    setTimeout(function() {
			                      ...
			                      window.location.href='<?php echo base_url();?>steps/clBusiness';
			                }, 2000);
